  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-ink-400">Chat with our support team. We're here to help.</p>
      </div>

      <div className="card flex h-[calc(100vh-16rem)] min-h-[400px] flex-col !p-0">
        {/* Chat header */}
        <div className="flex items-center gap-3 border-b border-ink-700 px-5 py-4">
          <div className="relative">
            <div className="grid h-10 w-10 place-items-center rounded-full bg-brand-500/20">
              <HeadphonesIcon className="h-5 w-5 text-brand-400" />
            </div>
            <span className="absolute -bottom-0.5 -right-0.5 h-3 w-3 rounded-full border-2 border-ink-800 bg-green-400" />
          </div>
          <div>
            <p className="text-sm font-bold text-white">Adparlorr Support</p>
        ...
        <div className="flex flex-wrap gap-2 border-t border-ink-700 px-5 py-3">
          {quickPrompts.map((p) => (
            <button
              key={p}
              onClick={() => setInput(p)}
              className="rounded-full border border-ink-600 px-3 py-1.5 text-xs text-ink-300 transition hover:border-brand-500 hover:text-brand-300"
            >
              {p}
            </button>
          ))}
        ...
        {/* Input */}
        <form onSubmit={sendMessage} className="flex items-center gap-2 border-t border-ink-700 px-5 py-4">
          <input
            type="text"
            value={input}
      ...
      {supportSettings?.telegramEnabled && supportSettings.telegramUrl && (
        <div className="card flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
          <div>
            <h3 className="text-lg font-bold text-white">Prefer Telegram?</h3>
            <p className="mt-1 text-sm text-ink-400">Chat directly with our support team.</p>
          </div>
          <a
